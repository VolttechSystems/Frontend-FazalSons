...
const CustomerChannel = () => {
  ...
  //           customer_channel.id === editId
  //             ? { ...customer_channel, ...formData }
  //             : customer_channel,
  //         ),
  //       )
  //     } else {
  ...
  const handleAddOrUpdateChannel = async (e) => {
    ...
    try {
      if (isEditing) {
        const response = await Network.put(`${Urls.actionCustomerChannel}/${editId}/`, formData)
        if (!response.ok) return console.log(response.data.error)
        setChannels(
          channels.map((customer_channel) =>
            customer_channel.id === editId
        ...
        const response = await Network.post(Urls.addCustomerChannel, formData)
        if (!response.ok) return console.log(response.data.error)
        setChannels([...channels, response.data])
      }
      setFormData({ customer_channel: '' })
  ...
  const handleDelete = async (id) => {
    ...
    const response = await Network.delete(`${Urls.actionCustomerChannel}/${id}/`)
    if (!response.ok) return console.log(response.data.error)
    setChannels(channels.filter((customer_channel) => customer_channel.id !== id))
